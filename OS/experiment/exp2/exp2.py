...
def init(Random=0):  # 初始化进程，生成五个进程并按到达时间将它们放入就绪队列
    # 首先清空各个变量
    global CLOCK
    CLOCK = 0
    global STATE
    ...
    for i in range(PNUM):
        if Random:
            var[i][0].set(random.randint(0, 20))
            var[i][1].set(random.randint(5, 15))
        readyList.append(
            PCB(pidList[i], 1, var[i][0].get(), var[i][1].get(), 0, 0, 'Ready'))
        for j in range(1, 4):
            varLbP[i][j].set('NaN')
        bar[i]['value'] = 0
...
def sortProcessbyPriority(begin, clock):
    ...
    t = findBorder(clock)
    for i in range(t):
        readyList[i].priority = float(
            readyList[i].waitTime+readyList[i].serveTime)/readyList[i].serveTime
...
def showInfo(now):
    ...
    varLbP[pos][1].set(readyList[now].arrTime +
                       readyList[now].runTime+readyList[now].waitTime)
    varLbP[pos][2].set(readyList[now].runTime+readyList[now].waitTime)
    varLbP[pos][3].set(round(float(readyList[now].runTime +
                                   readyList[now].waitTime)/readyList[now].serveTime, 2))
...
def SJF(clock):  # 最短进程优先
    ...
    if(readyList[0].runTime >= readyList[0].serveTime):  # 当前进程已经完成
        showInfo(0)
        readyList.remove(readyList[0])  # 移除当前进程
        # 当前周期下已经全部执行完毕，再次进行排序应按照下一周期进行，因此clock+1
        sortProcessbyServeTime(0, clock+1)
...
def HRN(clock):  # 高响应权
    ...
    if(readyList[0].runTime >= readyList[0].serveTime):  # 当前进程已经完成
        ...
        readyList.remove(readyList[0])  # 移除当前进程
        # 当前周期下已经全部执行完毕，再次进行排序应按照下一周期进行，因此clock+1
        sortProcessbyPriority(0, clock+1)
...
lbList = []  # 输入进程Label
for i in range(PNUM):
    lbList.append(tk.Label(frmInput, text=pidList[i], font=(
        'Songti', 12)).grid(row=1+i, column=0, padx=15, pady=20))
...
for i in range(PNUM):
    ...
    spinn = []  # 存储内层spinbox
    for j in range(2):
        varr.append(tk.IntVar())  # 进程请求信息
        varr[j].set(initList[i][j])
        spinn.append(tk.Spinbox(frmInput, from_=0, to=15,
                                width=8, textvariable=varr[j]))  # 进程请求按钮
...
l = ['FCFS', 'SJF', 'RR', 'HRN']
for i in range(1, 5):
    tk.Radiobutton(frmControl, text=l[i-1], value=i,
                   variable=selected).place(relx=0.4, rely=0.1*i)

rateLabel = tk.Label(frmControl, text="速率", font=(
    'Songti', 12)).place(relx=0.3, rely=0.85)
rate = tk.Spinbox(frmControl, from_=0, to=100, width=7,
                  textvariable=RATE).place(relx=0.5, rely=0.85)  # 设置速率选择栏

# 设置功能选择栏
btnAllRun = tk.Button(frmControl, text="      执行到底     ",
                      command=lambda: allRun())
btnStepRun = tk.Button(frmControl, text="       步进执行     ",
                       command=lambda: stepRun())
btnStop = tk.Button(frmControl, text="     停止执行      ", command=lambda: stop())
btnReset = tk.Button(frmControl, text="复位并随机生成值", command=lambda: init(1))
btnAllRun.place(relx=0.1, rely=0.5)
btnStepRun.place(relx=0.6, rely=0.5)
btnStop.place(relx=0.1, rely=0.7)
